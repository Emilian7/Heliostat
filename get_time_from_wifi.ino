...
Adafruit_INA219 ina219(0x41);  // Use a different adress to prevent I2C problems
// Constants
...
void setup() {
  ...
  Serial.println(" ");
 /*
  if (voltage < 3.4) {  //Low voltage leads to longer sleep
    time_to_sleep = 1800;
...
*/
  ...
  Connect_To_Internet();
  delay(1000);
  //Both functions can be found below

  Calculate_Sun_Position(timeHour, timeMin, 0, timeDay, timeMonth, timeYear);  // calculates the suns current position relative to Malmö
  //Function is below
  if (sun_elevation < -40) {
  ...
  if (sun_elevation > 0) {
    ...
    pwm.begin();
    if (!pwm.begin()) {  // if the charge reader isn't detected
    Serial.println("Failed to find servo driver");
    while (1) { delay(10); }
  }
    ...
    int azimuth_pulseLenght = map(sun_azimuth, 0, 180, SERVOMAX, SERVOMIN);
    ...
    pwm.setPWM(3, 0, elevation_pulseLenght);  //one vertikal
    delay(1000);
    digitalWrite(powerControlPin, LOW);
    Serial.println("turing off 5V supply");
  }
  WiFi.disconnect();
    //WiFi.mode(WIFI_OFF);   // Disable WiFi completely
...
…‚
  ...
  ina219.setCalibration_16V_400mA();  //Callibrated to precise mesurments
  float current_mA = 0;
  ...
  float average_current = (total_current / samples);  // Total current divided by total readings

  Serial.print("charge current ");
  Serial.println(average_current);  //Reads charge current when not connedted to wifi
...
void Start_sleep() {  //Send the ESP into deep sleep - power saving mode
  Serial.flush();
  esp_sleep_enable_timer_wakeup(time_to_sleep * uS_TO_S_FACTOR);
  esp_deep_sleep_start();
}
void loop() {}  //Loop is needed for the program to run